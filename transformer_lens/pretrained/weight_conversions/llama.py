...
def convert_llama_weights(llama, cfg: HookedTransformerConfig):
    ...
    for l in range(cfg.n_layers):
        ...
        state_dict[f"blocks.{layer_idx}.attn.{gqa_uscore}W_K"] = W_K
        state_dict[f"blocks.{layer_idx}.attn.{gqa_uscore}W_V"] = W_V

        state_dict[f"blocks.{layer_idx}.attn.b_Q"] = torch.zeros(
            cfg.n_heads, cfg.d_head, dtype=cfg.dtype, device=cfg.device
        )
        state_dict[f"blocks.{layer_idx}.attn.{gqa_uscore}b_K"] = torch.zeros(
            n_kv_heads,
            cfg.d_head,
            dtype=cfg.dtype,
            device=cfg.device,
        )
        state_dict[f"blocks.{layer_idx}.attn.{gqa_uscore}b_V"] = torch.zeros(
            n_kv_heads,
            cfg.d_head,
            dtype=cfg.dtype,
            device=cfg.device,
        ...
        state_dict[f"blocks.{layer_idx}.attn.W_O"] = W_O.to(device=cfg.device)

        state_dict[f"blocks.{layer_idx}.attn.b_O"] = torch.zeros(
            cfg.d_model, dtype=cfg.dtype, device=cfg.device
        )

        state_dict[f"blocks.{layer_idx}.ln2.w"] = llama.model.layers[l].post_attention_layernorm.weight

        # in case of quantization,
        # parameters should stay as bitsandbytes.nn.modules.Params4bit
        if not cfg.load_in_4bit:
            state_dict[f"blocks.{layer_idx}.mlp.W_in"] = llama.model.layers[l].mlp.up_proj.weight.T
            state_dict[f"blocks.{layer_idx}.mlp.W_gate"] = llama.model.layers[l].mlp.gate_proj.weight.T
            state_dict[f"blocks.{layer_idx}.mlp.W_out"] = llama.model.layers[l].mlp.down_proj.weight.T
        else:
            state_dict[f"blocks.{layer_idx}.mlp.W_in"] = llama.model.layers[l].mlp.up_proj.weight
            state_dict[f"blocks.{layer_idx}.mlp.W_gate"] = llama.model.layers[l].mlp.gate_proj.weight
            state_dict[f"blocks.{layer_idx}.mlp.W_out"] = llama.model.layers[l].mlp.down_proj.weight
